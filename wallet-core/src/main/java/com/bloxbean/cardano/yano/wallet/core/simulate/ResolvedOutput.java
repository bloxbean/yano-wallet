...
/**
 * A transaction output resolved from an output reference (ADR-042) — what the
 * node's {@code GET /utxos/{txHash}/{index}} answers. This is what makes the
 * value diff possible: a transaction's inputs name outputs, and only a node with
 * the UTxO set can say whose they are and what they hold.
 */
