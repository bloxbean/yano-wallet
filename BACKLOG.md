...
| E19 | **Remove the chained-submit retry workaround** | **workaround shipped — [yano-wallet#2](https://github.com/bloxbean/yano-wallet/issues/2), blocked on [yano#66](https://github.com/bloxbean/yano/issues/66)** | Yano's mempool admission resolves inputs against the persisted UTxO set only, so a transaction spending an output of another transaction still in the mempool is rejected with `UtxoNotFound` for the ~20s until the parent reaches a block. cardano-node accepts chained transactions, so a dApp submitting a pair back to back works against every other submission path and fails only here — which is how a CIP-113 token registration broke on preview (2026-08-13), setup transaction in block 4566041 and the one spending its output rejected. `WalletCip30Wallet.submitTx` now retries, but only when the node said `UtxoNotFound` **and** the transaction spends an output this wallet submitted moments ago; it polls up to `CHAIN_WAIT` (45s) for the parent. Delete it — not just leave it — once the node fix is in the **minimum supported** Yano version, not merely the latest: left in place it still burns 45s on a genuinely missing input before returning the same error. The removal list is in the issue. |
